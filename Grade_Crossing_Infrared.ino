...
enum CROSSINGSTATES
{
  ...
  ST_FLASHING3,
  ST_FLASHING4,
}; //identifies our 5 crossing states
...
void crossingflashing1(int valA1, int valA2){
  ...
  else if (valA1>500 && valA2>500 && flashcount>5){
    crossingState=ST_FLASHING4; // flashing 4 keeps the signal flashing even when nothing is on a sensor this goes on until another sensor is tripped
  }
}
void crossingflashing2(int valA1, int valA2){
 ...
 void crossingflashing3(int valA1, int valA2){
  ...
  if (valA1<500 || valA2<500){
    ...
    crossingState=ST_OFF; // prevents infinte looping
  }
  }
